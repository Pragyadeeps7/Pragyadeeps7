...
import { Star, Heart, Minus, Plus, ShoppingBag, Truck, RefreshCw, ShieldCheck, ChevronRight } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-6">
        <nav className="flex items-center gap-2 text-[12px] tracking-[0.18em]" style={{ color: "#8A8A8A" }}>
      ...
      <section className="max-w-[1440px] mx-auto px-6 lg:px-10 grid lg:grid-cols-2 gap-12 pb-16">
        <div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[0,1,2,3].map(i => (
              <div key={i} className="aspect-square bg-[#F5F0E5] cursor-pointer border" style={{ borderColor: i === 0 ? "#1A1A1A" : "transparent" }}>
        ...
        <div className="py-2">
          ...
          <h1 className="font-serif-display text-[34px] lg:text-[44px] leading-tight mb-3" style={{ fontWeight: 500 }}>{product.name}</h1>
          ...
          <div className="flex items-baseline gap-3 mb-2">
            <span className="font-serif-display text-[30px]" style={{ fontWeight: 600 }}>{formatPrice(product.price)}</span>
            {product.compareAtPrice && (
              <span className="text-[16px] line-through" style={{ color: "#8A8A8A" }}>{formatPrice(product.compareAtPrice)}</span>
            )}
          </div>
          <p className="text-[12px] mb-7" style={{ color: "#8A8A8A" }}>Inclusive of all taxes · Free shipping over ₹2,000</p>

          <p className="text-[15px] font-body leading-relaxed mb-7" style={{ color: "#5C5C5C" }}>{product.description}</p>
            ...
            <div className="flex gap-3">
              {product.colors.map((c, i) => (
                <button key={i} onClick={() => setColor(i)}
                  className={`w-10 h-10 rounded-full border-2 transition-all ${color === i ? "ring-2 ring-offset-2 ring-[#D4A574]" : ""}`}
            ...
            <div className="flex gap-2">
              {product.sizes.map(s => (
                <button key={s} onClick={() => setSize(s)}
                  className={`w-12 h-12 border text-[14px] font-medium transition-colors ${size === s ? "bg-[#1A1A1A] text-white border-[#1A1A1A]" : ""}`}
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <button onClick={() => addToCart(product, qty)} className="btn-dark flex-1 flex items-center justify-center gap-2">
          ...
          <div className="grid grid-cols-3 gap-3 py-5 border-t border-b" style={{ borderColor: "#EFE7D6" }}>
            {[
          ...
          <div className="mt-8">
            <div className="flex gap-6 border-b" style={{ borderColor: "#EFE7D6" }}>
              {["details", "shipping", "reviews"].map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`pb-3 text-[12px] tracking-[0.22em] uppercase font-medium ${tab === t ? "border-b-2 border-[#1A1A1A]" : ""}`}
                  style={{ color: tab === t ? "#1A1A1A" : "#8A8A8A" }}>
                  {t}
                </button>
              ))}
            </div>
            <div className="py-5 text-[14px] leading-relaxed" style={{ color: "#5C5C5C" }}>
              ...
              {tab === "shipping" && (
                <p>Dispatched in 1-3 working days. Free shipping on orders above ₹2,000. We use protective packaging to ensure each piece arrives in perfect condition.</p>
              )}
              {tab === "reviews" && (
                <p>{product.reviews} verified reviews  Average rating {product.rating}/5. Patrons love the craftsmanship and the elevated everyday feel.</p>
              )}
            </div>
          </div>
        </div>
      ...
      <section className="py-16" style={{ background: "#FAFAF7" }}>
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
          <h3 className="font-serif-display text-[28px] mb-10 text-center">You may also love</h3>
